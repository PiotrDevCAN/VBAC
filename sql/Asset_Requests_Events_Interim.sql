...
CREATE VIEW "VBAC_UT"."ASSET_REQUESTS_EVENTS_INTERIM" ("REF", "CREATED_IN_VBAC", "REJECTED_IN_VBAC","AWAITING_IAM", "APPROVED_FOR_ORDER_IT", "PRE_REQ_CREATED", "PRE_REQ_APPROVED", "EXPORTED_FOR_ORDER_IT", "RAISED_IN_ORDER_IT", "APPROVED_IN_ORDER_IT", "REJECTED_IN_ORDER_IT", "PROVISIONED_BY_ORDER_IT","ORDER_IT_RESPONDED") AS
...
DROP VIEW "VBAC"."ASSET_REQUESTS_EVENTS_INTERIM";

CREATE VIEW "VBAC"."ASSET_REQUESTS_EVENTS_INTERIM" ("REF", "CREATED_IN_VBAC", "REJECTED_IN_VBAC","AWAITING_IAM", "APPROVED_FOR_ORDER_IT", "PRE_REQ_CREATED", "PRE_REQ_APPROVED", "EXPORTED_FOR_ORDER_IT", "RAISED_IN_ORDER_IT", "APPROVED_IN_ORDER_IT", "REJECTED_IN_ORDER_IT", "PROVISIONED_BY_ORDER_IT","ORDER_IT_RESPONDED") AS
select request_reference as ref,
case when event = 'Created in vBAC' then occured else null end  as created_in_vbac
,case when event = 'Rejected in vBAC' then occured else null end as rejected_in_vbac
,case when event = 'Awaiting IAM Approval' then occured else null end as awaiting_iam
...
GRANT CONTROL ON TABLE "VBAC"."ASSET_REQUESTS_EVENTS_INTERIM" TO USER "BLUADMIN";

GRANT DELETE ON TABLE "VBAC"."ASSET_REQUESTS_EVENTS_INTERIM" TO USER "BLUADMIN" WITH GRANT OPTION;


GRANT INSERT ON TABLE "VBAC"."ASSET_REQUESTS_EVENTS_INTERIM" TO USER "BLUADMIN" WITH GRANT OPTION;

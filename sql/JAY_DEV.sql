CREATE TABLE "JAY_DEV"."DB2_ERRORS" (
		"USERID" CHAR(50) DEFAULT NULL,
		"PAGE" VARCHAR(200) DEFAULT NULL,
		"DB2_ERROR" CHAR(10) DEFAULT NULL,
		"DB2_MESSAGE" CHAR(200) DEFAULT NULL,
		"BACKTRACE" VARCHAR(1024) DEFAULT NULL,
		"REQUEST" VARCHAR(1024) DEFAULT NULL,
		"TIMESTAMP" TIMESTAMP NOT NULL
	)
	DATA CAPTURE NONE;

CREATE TABLE "JAY_DEV"."PERSON" (
		"CNUM" INTEGER NOT NULL,
		"OPEN_SEAT_NUMBER" INTEGER,
		"FIRST_NAME" CHAR(20),
		"LAST_NAME" CHAR(20),
		"PHONE_NUMBER" INTEGER,
		"EMAIL_ADDRESS" CHAR(40),
		"LBG_EMAIL" CHAR(40),
		"EMPLOYEE_TYPE" CHAR(20),
		"FM_CNUM" INTEGER,
		"FM_MANAGER_FLAG" CHAR(3),
		"CTB_RTB" CHAR(10),
		"TT_BAU" CHAR(3),
		"LOB" CHAR(40),
		"ROLE_ON_THE_ACCOUNT" CHAR(40),
		"ROLE_TECHNOLOGY" CHAR(40),
		"START_DATE" TIMESTAMP,
		"PROJECTED_END_DATE" TIMESTAMP,
		"THIRD_PARTY_VDI_ACCESS" CHAR(3),
		"EXTERNAL_COMPANY" CHAR(40),
		"COUNTRY" CHAR(40),
		"CT_ID" INTEGER,
		"CT_REQUEST" TIMESTAMP,
		"CYBERARK" CHAR(3),
		"FM_APPROVAL" CHAR(10),
		"FULL_VPN" CHAR(3),
		"IBM_BASE_LOCATION" VARCHAR(512),
		"LBG_LOCATION" VARCHAR(512),
		"LAPTOP_ASSET" CHAR(3),
		"MPW_ACCOUNT_TYPE" CHAR(10),
		"MPW_DOMAIN" CHAR(20),
		"MPW_ID" INTEGER,
		"OCS" CHAR(3),
		"OFFBOARDED_DATE" TIMESTAMP,
		"PES_DATE_REQUESTED" TIMESTAMP,
		"PES_DATE_RESPONDED" TIMESTAMP,
		"PES_DETAILS" CHAR(3),
		"PES_STATUS" CHAR(10),
		"RAS_ACCESS" CHAR(3),
		"REVALIDATION_DATE_FIELD" TIMESTAMP,
		"REVALIDATION_STATUS" CHAR(10),
		"SDID" CHAR(3),
		"VDI" CHAR(3),
		"VPN_LITE" CHAR(3),
		"WORK_STREAM" CHAR(40)
	)
	DATA CAPTURE NONE;

CREATE TABLE "JAY_DEV"."TRACE" (
		"LOG_ENTRY" VARCHAR(32000) NOT NULL,
		"LASTUPDATER" CHAR(50) NOT NULL,
		"LASTUPDATED" TIMESTAMP NOT NULL DEFAULT CURRENT TIMESTAMP,
		"CLASS" CHAR(50) DEFAULT NULL,
		"METHOD" CHAR(50) DEFAULT NULL,
		"PAGE" VARCHAR(200) DEFAULT NULL,
		"ELAPSED" DOUBLE DEFAULT NULL
	)
	DATA CAPTURE NONE;

CREATE TABLE "JAY_DEV"."TRACE_CONTROL" (
		"TRACE_CONTROL_TYPE" CHAR(20) DEFAULT NULL,
		"TRACE_CONTROL_VALUE" CHAR(40) DEFAULT NULL
	)
	DATA CAPTURE NONE;

ALTER TABLE "JAY_DEV"."PERSON" ADD CONSTRAINT "SQL171213120532360" PRIMARY KEY
	("CNUM");

CREATE TABLE "JAY_DEV"."STATIC_GROUPS" (
		"GROUP_ID" INTEGER NOT NULL GENERATED BY DEFAULT AS IDENTITY ( START WITH 1 INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 NO CYCLE CACHE 20),
		"GROUP" CHAR(100) NOT NULL
	)
	DATA CAPTURE NONE;

ALTER TABLE "JAY_DEV"."STATIC_GROUPS" ADD CONSTRAINT "Q_GROUP_GROU00001_00001" PRIMARY KEY
	("GROUP_ID");




ALTER TABLE "JAY_DEV"."ROLES" DROP CONSTRAINT "Q_REST_RESOU00001_RESOU00001_00001";

DROP TABLE "JAY_DEV"."ROLES";

CREATE TABLE "JAY_DEV"."STATIC_ROLES" (
		"ROLE_ID" INTEGER NOT NULL GENERATED BY DEFAULT AS IDENTITY ( START WITH 1 INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 NO CYCLE CACHE 20),
		"ROLE_TITLE" CHAR(100) NOT NULL
	)
	DATA CAPTURE NONE;

ALTER TABLE "JAY_DEV"."STATIC_ROLES" ADD CONSTRAINT "Q_REST_RESOU00001_RESOU00001_00001" PRIMARY KEY
	("ROLE_ID");




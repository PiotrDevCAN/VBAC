    ...
    function displayForm(){
        ...
        switch (true){
            ...
            case $isPmo:
                ...
                break;
            case $isFm:
                $predicate = " ( FM_CNUM='" . htmlspecialchars($myCnum) . "' or CNUM='" . htmlspecialchars($myCnum) . "' $iAmDelegateForPredicate ) ";
            break;
            default:
                $predicate = " ( CNUM='" . htmlspecialchars($myCnum) . "' $iAmDelegateForPredicate  ) ";
            break;
        }
...
<!-- 				<p class='bg-warning'><small>The ability to initiate the return of an asset has been suspended. Please contact PMO direct to arrange</small></p> -->
    ...
    function createJsCtidLookup(){
        $loader = new Loader();
        $ctIdCnum = $loader->loadIndexed('CT_ID','CNUM',allTables::$PERSON);
        $ctIdWorkerId = $loader->loadIndexed('CT_ID','WORKER_ID',allTables::$PERSON);
        $ctbFlag = $loader->loadIndexed('CTB_RTB','CNUM',allTables::$PERSON);
        JavaScript::buildObjectFromLoadIndexedPair($ctIdCnum,'cnum2ctid');
        JavaScript::buildObjectFromLoadIndexedPair($ctIdWorkerId,'workerId2ctid');

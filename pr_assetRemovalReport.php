...
->setTitle('ODC Asset Removal : generated from vBAC')
->setSubject('ODC Asset Removal')
->setDescription('Report of people with the right to remove assets from the ODC')
->setKeywords('office 2007 openxml php vbac odc')
->setCategory('ODC_ASSET_REMOVAL based report');
// Add some data

$now = new DateTime();
$odcAssetRemovalTable = new odcAssetRemovalTable(allTables::$ODC_ASSET_REMOVAL);
...
try {
    ...
    if($rs){
        ...
        if($recordsFound){
            ...
            $spreadsheet->getActiveSheet()->setTitle('Asset Removal');
            DbTable::autoSizeColumns($spreadsheet);

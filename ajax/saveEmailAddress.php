...
$workerId    = !empty($_POST['eam_workerid']) ? $_POST['eam_workerid'] : null;
$email   = !empty($_POST['eam_email']) ? $_POST['eam_email'] : null;
$field   = !empty($_POST['eam_field']) ? $_POST['eam_field'] : null;

if (!empty($cnum) && !empty($workerId) && !empty($email) && !empty($field) ) {
    switch ($field){
        case 'EMAIL_ADDRESS':
        case 'IBM_EMAIL_ADDRESS':
        case 'KYN_EMAIL_ADDRESS':
            $personTable = new personTable(allTables::$PERSON);
            $personTable->setEmailField($cnum, $workerId, $field, $email);
            break;
        default:

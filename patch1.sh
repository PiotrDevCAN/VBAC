...
yum update -y && yum install -y php-pear php-devel
mkdir -p /opt/ibm
sed -i 's:variables_order = "GPCS":variables_order = "GPCES":g' /etc/php.ini

rm -rf /var/cache/yum && yum remove -y nodejs && yum clean all && yum update -y && yum upgrade -y
# yum install unzip zip libpng-devel libjpeg-turbo-devel postgresql-devel
